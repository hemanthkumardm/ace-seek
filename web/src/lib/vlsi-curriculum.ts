...
export const LEARN_TRACKS: LearnTrack[] = [
  ...
  },
  {
    ...
    blurb:
      "Tech-node kits, Liberty anatomy, Tech/Macro LEF, DEF/GDS/CDL, SPEF extraction, PDK QA — tied to OpenROAD Studio.",
    accent: "emerald",
  },
  {
    id: "xml",
...
write_verilog -output gate_netlist.v`,
  ...
  },
  {
    ...
    kind: "theory",
    layer: "beginner",
    title: "How Synthesis Uses .lib (Pointer to PDK Track)",
    minutes: 10,
    level: "working",
    summary:
      "Synthesis links RTL to characterized standard cells. Deep Liberty/LEF/PDK formats live in the PDK & EDA Formats course — this lesson stays synthesis-focused.",
    body: [
      "Synthesis needs three inputs: RTL, **target/link Liberty (`.lib` / `.db`)**, and **SDC**. The `.lib` supplies cell area, pin capacitance, timing arcs, and power so mapping and optimization are legal.",
      "Drive strengths (`INV_X1`…`X8`) trade delay vs load on the previous stage — synthesis picks sizes under SDC.",
      "Pre-layout timing uses wire-load or early physical estimates; signoff uses SPEF + liberty corners (see MMMC + PDK tracks).",
      "**Do not re-learn formats here.** Open the dedicated course **PDK & EDA Formats** (`/vlsi/learn/c/pdk`) for annotated `.lib` / Tech LEF / Macro LEF / DEF / GDS / SPEF and tech-node packaging (sky130 vs ORFS).",
      "Studio practice: after reading PDK lessons, pick a PDK on `/openroad/project` and export a script pack — confirm liberty basenames match the kit.",
    ],
  },
  {
    slug: "synth-video",
