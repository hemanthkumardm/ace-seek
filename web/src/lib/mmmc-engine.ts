...
/** Advanced FinFET dual-corner func MMMC reference style */
export function innovusReferenceStyleState(): MmmcState {
  ...
  const rcBest = createRcCorner({
    ...
    name: "cbest_125c",
    temperatureC: 125,
    qrcTechFile: "./tech/qrc_cbest.tch",
  });
  const rcWorst = createRcCorner({
    id: "rc_rcworst",
    name: "rcworst_m40c",
    temperatureC: -40,
    qrcTechFile: "./tech/qrc_rcworst.tch",
  });
  const opFf = createOpCond({
    id: "op_ff",
    name: "ffgnp_op_best_125c",
...
export const MMMC_PRESETS: {
  ...
  {
    name: "Innovus dual-corner func (Open-Source FinFET style)",
    description:
...
export function generateMmmcTcl(
  ...
  lines.push("# Generated by Ace-Seek VLSI Portal · PLAN-compatible objects");
  lines.push("# Genus:  read_mmmc this_file.tcl");
  lines.push("# Innovus/Tempus: source this_file.tcl  (or read_mmmc)");
  lines.push("################################################################################");
  lines.push("");
