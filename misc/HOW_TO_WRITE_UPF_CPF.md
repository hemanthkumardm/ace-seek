...
```mermaid
flowchart TB
  subgraph AO["PD_AO / always-on  VDD VSS"]
    PADS[Pads / wake / glue]
    SW[Power switch]
  end
  subgraph CORE["PD_CORE switchable  VDD_CORE"]
    LOGIC[Block logic]
  end
  VDD --> SW
  SW -->|on| VDD_CORE
  LOGIC -->|outputs| ISO[Isolation clamp]
  ISO --> PADS
  PADS -->|inputs| LOGIC
```
...
```mermaid
flowchart LR
  V[upf_version] --> S[supplies]
  S --> D[domains]
  D --> SW[switches]
  SW --> ISO[isolation]
  ISO --> LS[level shifters<br/>if multi-V]
  LS --> RET[retention<br/>if needed]
  RET --> PST[PST / states]
  PST --> PA[port attributes]
```
